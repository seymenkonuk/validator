...
class UploadedFileValidator extends BaseValidator
{
    ...
    public function validate(mixed $data, bool $exists = true): ValidationResult
    {
        ...
        // Doğru Formatta Array Gelmiş Olmalı!
        if (
            !is_array($data)
        ...
        if ($this->allowedMimes !== null) {
            ...
            // Dosya İçeriğine Göre Türü Kontrol Et
        ...
        foreach ($this->regexRules as $regexRule) {
            if (preg_match($regexRule["pattern"], $data["name"]) !== 1) {
